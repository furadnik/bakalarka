...
For this reason, we only present the results for $ n=5 $, for which the optimal algorithms took approximately 800 CPU hours to compute.
Using the complexity bound from \Cref{thm:algfo-time-complexity-double-exp}, we estimate that for $ n=6 $, to compute the optimal algorithms for all choices of $ \tau $, it would require over 1000 CPU years with the computational resources currently available to us.
