...
Using the complexity bound from \Cref{thm:algfo-time-complexity-double-exp}, we estimate that for $ n=6 $, to compute the optimal algorithms for all choices of $ \tau $, it would require over 100 years with our current setup\david{infrastructure? Setup je trochu lidové, infrastructure se mi ale taky moc nelíbí...}.
...
\filip{essentially stejnej paragraf mam i v preliminaries. jeden chceme vyhodit?}
\david{nevyhazoval bych to, přijde mi to ok připomenout proč to děláme a ty důvody by se neměly měnit. Navíc už je to dávno co jsme to posledně viděli}
...
We have previously seen (\Cref{sec:supermodular}) that for $ n=5 $, all algorithms perform well, while behaving all in a very similar way -- first choosing the $ n $ largest subsets.
