...
We first show results utilizing the game-theoretic motivation, with the utopian gap (\Cref{def:gap}) as\david{set} the divergence.
...
Using the complexity bound from \Cref{thm:algfo-time-complexity-double-exp}, we estimate that for $ n=6 $, to compute the optimal algorithms for all choices of $ \tau $, it would require over 100 years with our current computational resources. \filip{je "computational resources" dobry? bylo "setup", david nadhazoval jeste "infrastructure"...}\filip{jeste navrhuju "computing power"}\david{za mě co je teď dobrý, ale jiné možnosti taky fajn}
