...
The performance of each algorithm changes, becoming less predictible.\filip{eeeeeh}
We suspect that this change is due to the nonlinearity of the $ L_2 $ norm.
However, we do see that the relative performance of each algorithm did not change much.
Interestingly, the gap between the performances of the offline and oracle approaches seems to be widening.
This allows us to see more clearly that the PPO algorithm, as we theorised, starts by behaving similarly to the offline approaches, and later reaches the performance of the oracle approaches.
