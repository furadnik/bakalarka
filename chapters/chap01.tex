...
Supermodularity is widely studied in various fields, most notably in cooperative game theory (which we will discuss below) and economics.\filip{TODO: citace}\martin{Neřekl bych broadly economics. Dá se asi říct všechno, kde je sub i super modularity, tj. coop hry, combinatorial auctions, matroid theory a combinatorial optimizations: https://link.springer.com/chapter/10.1007/978-3-642-68874-4_10 na comb. optimization a zbytek můžeš použít zdroje, které jsem uvedl výše}
...
Other notions of distribution where the payoff vector needn't \martin{need not, neměli by se psát zkrácené výrazy} be efficient are sometimes studied, but we will not discuss them here.\david{jako nevim jestli nás tohle vůbec trápí} % TODO: citace
\filip{hele já jsem to malinko pošťouchal a nechal bych to tady takhle...}\david{može být, co takhle přihodit nějaké citace na ty non-efficient koncepty / články?}\martin{jeden ze základních je asi: https://www.researchgate.net/publication/226335741_Cooperative_Games_with_Coalition_Structures, ikdyž je efficient na koaliční struktuře teda... }
...
Hence, assuming perfect rationality of the players, it won't form.
...
It uses two deep neural networks, the \emph{actor} and the \emph{critic}.\footnote{Note that there are also variations of PPO approximating the actor and the critic using other means than neural networks. We will not discuss them further in this thesis.}
