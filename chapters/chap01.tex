...
\filip{chci ty citace mit takhle najednou, nebo to rozdelit a dat neco k matroidum, neco k utilite atd?}
\david{obojí je fajn, ale rozhodit je za mě lepší}
...
\filip{co říkáte teď na tenhle odstavec?}\david{za mě good}
...
\begin{prop}[Characterization of the Shapley value {\citep{Shapley+1953+307+318}}]
	The Shapley value $ \Shapley $ is the unique \david{Mistře, není unique linear?}\filip{podle mě to ne, linearitu zajišťuje poslední axiom}\david{na linearitu je ale ještě třeba $\phi_i(\alpha v) = \alpha \phi_i(v)$ ne? To nevim kde člověk dostane.} function satisfying the following properties: \begin{enumerate}[ ]
...
\filip{Co tohle? Asi po padesáté přepsáno, a už se mi to skoro začíná líbit :D}\david{až na ten začátek je to moc pěkné}
Another popular choice \david{of what? Možní another popular RL algorithm?} is the REINFORCE algorithm with baseline \cite[][Chapter 13]{sutton2018reinforcement}\filip{tady cituju suttona... je to ten algoritmus co ma i straca ve slidech}\david{tak to musí být správně :D}.
