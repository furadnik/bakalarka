...
We will denote the time complexity of computing the divergence for the specific choice of $ \ell $ as $ \fce\divt{n} $, a function of $ n $, the size of the ground set.
...
Then for $ n=5 $, this process takes $ 60 $ hours on a CPU (details are discussed in \Cref{chap:results}), and for $ n=6 $, we estimate it would take over $ 100 $ years.
...
\begin{figure*}[t!]
    ...
    \node[stff]        (RG)         [left=of FO]                  {\algRG{}};
    \node[stff,dotted]        (Online)     [right=of FO]                  {\emph{Online solution}};
    \node[stff]        (FG)         [below=of FO]                  {\algFG{}};

		%Lines
		\draw[->, very thick] (RG.north) to (RO.south);
		\draw[->, very thick] (FG.north) to (FO.south);
		\draw[->, very thick] (FO.north) to (RO.south);
		\draw[->, very thick, dotted] (Online.north) to (RO.south);
		\draw[->, very thick, dotted] (FO.east) to (Online.west);
	\end{tikzpicture}
	\caption{Comparison of the performance of different algorithms.
  The relation $ A \to B $ indicates that the resulting divergence of algorithm $ A $ is always higher than algorithm $ B $, in other words $ A $ is provably worse than $ B $.}
\end{figure*}
