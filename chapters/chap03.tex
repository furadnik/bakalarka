...
This approach doesn't guarantee the optimal solution, however, with $ \sigma $ large enough, the approximation converges to the optimal solution due to the law of large numbers \citep{Bernoulli1713}.
...
Then for $ n=5 $, this process takes $ 30 $ hours on a CPU (details are discussed in \Cref{chap:results}), and for $ n=6 $, we estimate it would take over $ 100 $ years.
...
The domain of the online problem then is the set of all policies, which is uncountably infinite, and due to that fact, an exhaustive search becomes infeasible.
...
To get a solution of the online problem which is applicable in practice\david{tohle mi nedává smysl, spíš něco jako "while being able to use it in practice"?}\martin{vyřešte to, zní to divně}\filip{co takhle?} in practice, we thus turn to \emph{reinforcement learning}.

% PPO state space
We employ the PPO algorithm, which we have described in detail in the \nameref{sec:prel-ppo}~\namecref{sec:prel-ppo} of Preliminaries (\Cref{sec:prel-ppo}).
...
This captures the state space as defined in the \nameref{def:online-pp} (\Cref{def:online-pp}).
...
However, as the experimental results show, the greedy approaches are generally very close to the optimum, and thus PPO works well in practice.
