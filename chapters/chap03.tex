...
\begin{figure*}[t!]
  ...
  The relation $ A \to B $ indicates that the resulting divergence of algorithm $ A $ is always higher than algorithm $ B $,\david{tečka?} in other words $ A $ is provably worse than $ B $.
  Dotted lines represent the relationship of\david{to?} the theoretical online optimal solution.\david{higher $->$ no smaller}
}
\end{figure*}
