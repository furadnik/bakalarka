...
% pseudokód \algFO{}
\david{furt nevim, jestli se mi líbí tenhle styl algoritmu. Je to na tobě, ale co tam přidat nějaké horizontal lines, ať se to odliší od textu?}
\begin{algor}{\label{alg:fo}\algFO{}}{Function distribution $ \fdist $, number of samples $ \sigma $, the initially known subsets $ \k \supseteq \k_0 $, the remaining budget $ \tau $, a divergence function $ \ell $.}
  ...
  \Indent For $ i \in \left\{ 1, 2, \ldots, \sigma \right\} $:
    ...
    \EndIndent
  \EndIndent
  \State $ \hat{\s_*} \gets \argmin_{\s\david{\in 2^N?}} D[\s] $
\end{algor}
...
Then for $ n=5 $, this process takes $ 800 $ CPU hours (details are discussed in \Cref{chap:results}), and for $ n=6 $, we estimate it would take over $ 1000 $ CPU years.
...
\begin{prop}[\algFG{} time complexity \david{of OFFLINE GREADY?}]
  Let $ n $ be the size of the ground set, $ \kappa $ the size of the initial information, $ \tau $ the remaining budget, $ \sigma $ the number of samples, and $ \ell $ a divergence function.
