...
We can see that, even for a large $ \tau $ and small $ \kappa $, the \algFG{} algorithm has a worst-case time complexity $ \fce{\Theta}{\sigma \cdot 2^{2n} \cdot \fce{\divt}{n}} $, which is much more manageable than $ \fce{\Theta}{\sigma \cdot 2^{2^n} \cdot 2^{-\frac{3n}{2}}\cdot \fce{\divt}{n}} $ of the \algFO{} algorithm.
...
\begin{figure*}[t!]
  ...
  \filip{jo transitivita šipky je implicitně předpokládaná... možná to zmínit v tom popisku? mně když jsem to vytvářel to přišlo relativně jasný, tak jsem to tam nepsal, ale nevim}
}
\end{figure*}
