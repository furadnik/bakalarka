...
To approximate the solution of the online problem, we thus turn to reinforcement learning.
...
Finally, we run experiments on the $ \factory[5] $ and $ \supermodular[5] $ distributions.
...
In the experiments, with increasing budget, PPO reaches the theoretical best-case performance bound of the \algRO{}.
Still, PPO is usable in practice, even for larger instances.
...
% To draw further conclusions about the empirical findings, we would require computing power unreachable to us at the current time.
% By the very nature of this problem, the space in which we search for solutions is up to double-exponential, as we show in \Cref{chap:pp}.
