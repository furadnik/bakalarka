...
% hence, in conclusin, vyřešili jsme život?! HA! tak já už musim. filip
	...
	However, in\david{není lepší when?} applying set functions to a real-world problem, there is a significant roadblock: their size grows exponentially in size of the ground set,\david{tečka?} while finding out the value of even a single subset might be hard -- costing, e.g., money, time, or computational power.
	...
	We establish a measure for the ambiguity arising from the unknown values, and we show its properties.
	...
	Further, we define a very simple heuristic which drastically decreases our ambiguity metric on the supermodular class \david{while only requiring a linear number of values to be known? }with only linear information.
}
