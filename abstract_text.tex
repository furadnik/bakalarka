...
% hence, in conclusin, vyřešili jsme život?! HA! tak já už musim. filip
	...
	However, in applying set functions to a real-world problem, there is a significant roadblock: their size grows exponentially in size of the ground set, while finding out the value of even a single subset might be hard -- costing, e.g., money, time, or computational power.
	...
	Further, we define a very simple heuristic which drastically decreases our ambiguity metric on the supermodular class with only linear information.
}
